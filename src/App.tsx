...
const facetFiltersToExpression = (value: string) => value.split('\n').map((line) => parseList(line).map((token) => {
  const separator = token.indexOf(':')
  if (separator <= 0) return ''
  const field = token.slice(0, separator).trim()
  const rawValue = token.slice(separator + 1).trim().replace(/^['"]|['"]$/g, '')
  if (!field || !rawValue) return ''
  const safeValue = rawValue.replace(/\\/g, '\\\\').replace(/"/g, '\\"')
  const literal = /^(true|false|null|-?\d+(\.\d+)?)$/i.test(rawValue) ? rawValue : `"${safeValue}"`
  return `${field} = ${literal}`
}).filter(Boolean)).filter((group) => group.length).map((group) => group.length > 1 ? `(${group.join(' OR ')})` : group[0]).join(' AND ')
...
function App() {
  ...
  const columns = useMemo(() => {
    ...
    hits.forEach((hit) => Object.keys(hit).filter((key) => key !== '_formatted').forEach((key) => keys.add(key)))
  ...
  const buildSearchParams = (pageNumber = page): SearchParams => {
    ...
    if (returnHighlight) {
      ...
      params.attributesToHighlight = highlightFields.length ? highlightFields : ['*']
    }
    ...
    if (returnHighlight) { params.highlightPreTag = '<mark>'; params.highlightPostTag = '</mark>' }
    ...
    <div className="app-shell">
      ...
      <main className="workspace">
        <section className="search-header">
          ...
          <div className="global-search-bar"><Search size={20} /><input id="query" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1) }} onKeyDown={(event) => event.key === 'Enter' && runSearch()} placeholder="搜索索引中的文档" /><label className="header-checkbox"><input type="checkbox" checked={returnHighlight} onChange={(event) => setReturnHighlight(event.target.checked)} /><span>高亮返回</span></label><kbd>↵</kbd><button onClick={() => runSearch()} disabled={searching || !selectedUid}>{searching ? <LoaderCircle className="spin" size={16} /> : <Search size={16} />}搜索</button></div>
        </section>
...
function renderCell(value: unknown, formattedValue?: unknown) { if (formattedValue !== undefined && typeof formattedValue === 'string' && formattedValue.includes('<mark>')) { const safe = formattedValue.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/&lt;mark&gt;/g, '<mark>').replace(/&lt;\/mark&gt;/g, '</mark>'); return <span title={formattedValue} dangerouslySetInnerHTML={{ __html: safe.length > 120 ? `${safe.slice(0, 120)}…` : safe }} /> } if (value === null || value === undefined) return <span className="null-cell">null</span>; if (typeof value === 'object') return <span className="object-cell">{Array.isArray(value) ? `[${value.length} 项]` : '{…}'}</span>; const text = String(value); return <span title={text}>{text.length > 90 ? `${text.slice(0, 90)}…` : text}</span> }
